...
fn empty_co() -> Co<OneEffect, ()> {
    Co::new(|_y| async move {})
}

fn single_yield_co() -> Co<OneEffect, ()> {
    Co::new(|y| async move {
        y.yield_(Noop).await;
    })
}

fn multi_yield_co(n: usize) -> Co<OneEffect, ()> {
...
fn dispatch_first_co() -> Co<FiveEffects, ()> {
    Co::new(|y| async move {
        y.yield_(Noop).await;
    })
}

fn dispatch_middle_co() -> Co<FiveEffects, ()> {
    Co::new(|y| async move {
        y.yield_(Alloc).await;
    })
}

fn dispatch_last_co() -> Co<FiveEffects, ()> {
    Co::new(|y| async move {
        y.yield_(Decrement).await;
    })
}

fn alloc_co() -> Co<ThreeEffects, ()> {
    Co::new(|y| async move {
        let _ = y.yield_(Alloc).await;
    })
}

fn stateful_co() -> Co<ThreeEffects, u64> {
    Co::new(|y| async move {
...
mod sync_benches {
    ...
    fn empty_coroutine(bencher: divan::Bencher) {
        bencher.bench(|| {
            let co = empty_co();
            let mut handler = hlist![|_: Noop| -> CoControl<OneEffect> { CoControl::resume(()) }];
    ...
    fn stateful_handler(bencher: divan::Bencher) {
        bencher.bench(|| {
            ...
            let mut handler = hlist![
                |_s: &mut u64, _: Noop| -> CoControl<ThreeEffects> { CoControl::resume(()) },
                |s: &mut u64, _: GetValue| -> CoControl<ThreeEffects> { CoControl::resume(*s) },
                |_s: &mut u64, _: Alloc| -> CoControl<ThreeEffects> {
    ...
    fn refcell_pattern(bencher: divan::Bencher) {
        bencher.bench(|| {
            ...
            let mut handler = hlist![
                |_: Noop| -> CoControl<ThreeEffects> { CoControl::resume(()) },
                |_: GetValue| -> CoControl<ThreeEffects> { CoControl::resume(*state.borrow()) },
                |_: Alloc| -> CoControl<ThreeEffects> { CoControl::resume(String::new()) },
            ];
            divan::black_box(corophage::sync::run(co, &mut handler))
        });
    }
    ...
    fn single_yield(bencher: divan::Bencher) {
        ...
        bencher.bench(|| {
            rt.block_on(async {
                let co = single_yield_co();
                let mut handler =
                    hlist![async |_: Noop| -> CoControl<OneEffect> { CoControl::resume(()) }];
    ...
    fn stateful_handler(bencher: divan::Bencher) {
        ...
        bencher.bench(|| {
            rt.block_on(async {
                ...
                let mut state = 42u64;
                let mut handler = hlist![
                    async |_s: &mut u64, _: Noop| -> CoControl<ThreeEffects> {
                        CoControl::resume(())
                    },
                    async |s: &mut u64, _: GetValue| -> CoControl<ThreeEffects> {
                        CoControl::resume(*s)
                    },
                    async |_s: &mut u64, _: Alloc| -> CoControl<ThreeEffects> {
                        CoControl::resume(String::new())
                    },
                ];
...
mod dispatch_benches {
    ...
    fn five_effect_handler() -> frunk::HList![
        impl FnMut(Noop) -> CoControl<FiveEffects>,
        impl FnMut(GetValue) -> CoControl<FiveEffects>,
        impl FnMut(Alloc) -> CoControl<FiveEffects>,
        impl FnMut(Increment) -> CoControl<FiveEffects>,
        impl FnMut(Decrement) -> CoControl<FiveEffects>,
    ] {
        hlist![
            |_: Noop| -> CoControl<FiveEffects> { CoControl::resume(()) },
            |_: GetValue| -> CoControl<FiveEffects> { CoControl::resume(42) },
            |_: Alloc| -> CoControl<FiveEffects> { CoControl::resume(String::new()) },
            |_: Increment| -> CoControl<FiveEffects> { CoControl::resume(IncrementResult) },
            |_: Decrement| -> CoControl<FiveEffects> { CoControl::resume(DecrementResult) },
        ]
    }
...
mod handler_complexity_benches {
    ...
    fn allocating_handler(bencher: divan::Bencher) {
        bencher.bench(|| {
            ...
            let mut handler = hlist![
                |_: Noop| -> CoControl<ThreeEffects> { CoControl::resume(()) },
                |_: GetValue| -> CoControl<ThreeEffects> { CoControl::resume(42) },
                |_: Alloc| -> CoControl<ThreeEffects> {
