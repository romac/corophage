...
fn greet<'a>() -> Effectful<'a, Effects![Ask, Print], ()> {
    Program::new(|mut y: Yielder<'_, Effects![Ask, Print]>| async move {
...
fn sync_invoke_subprogram_returns_value() {
    ...
    fn compute<'a>() -> Effectful<'a, Effects![Add], i32> {
        Program::new(|mut y: Yielder<'_, Effects![Add]>| async move { y.yield_(Add(1, 2)).await })
    }
...
fn sync_invoke_subprogram_subset_effects() {
    ...
    fn ask_name<'a>() -> Effectful<'a, Effects![Ask], &'static str> {
        Program::new(
            |mut y: Yielder<'_, Effects![Ask]>| async move { y.yield_(Ask("name?")).await },
        )
    }
...
fn sync_invoke_nested() {
    ...
    fn inner<'a>() -> Effectful<'a, Effects![Ask], &'static str> {
        Program::new(
            |mut y: Yielder<'_, Effects![Ask]>| async move { y.yield_(Ask("inner")).await },
        )
    }
