...
let result = Program::new(|y: Yielder<'_, Effects![Log<'static>, GetState<u64>, SetState<u64>]>| async move {
    y.yield_(Log("starting")).await;
    let val = y.yield_(GetState::default()).await;
    y.yield_(SetState(val * 2)).await;
    y.yield_(GetState::default()).await
})
